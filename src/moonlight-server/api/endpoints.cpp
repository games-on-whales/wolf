...
void UnixSocketServer::endpoint_StreamSessionResume(const HTTPRequest &req, std::shared_ptr<UnixSocket> socket) {
  auto resume_req = rfl::json::read<StreamSessionResumeRequest>(req.body);
  if (resume_req) {
    auto sessions = state_->app_state->running_sessions->load();
    auto session_id = std::stoul(resume_req.value().session_id);
    if (auto old_session = state::get_session_by_id(sessions.get(), session_id)) {
      // Copy, do not re-create. The Wayland display, the virtual input devices and the audio
      // sink are all shared_ptr members, so a copy keeps the SAME desktop and the same devices
      // already plugged into the container -- which is the whole point of resuming rather than
      // launching. Only the client-chosen crypto changes.
      //
      // Everything else is deliberately preserved, including rtsp_fake_ip and the RTP/ENet
      // secret payloads: those are generated by Wolf, not by the client, and rtsp_fake_ip is
      // how the RTSP thread identifies this session.
      auto new_session = std::make_shared<events::StreamSession>(old_session.value());
      new_session->aes_key = resume_req.value().aes_key.get();
      new_session->aes_iv = resume_req.value().aes_iv.get();

      state_->app_state->running_sessions->update(
          [&old_session, new_session](const immer::vector<events::StreamSession> &ses_v) {
            return state::remove_session(ses_v, old_session.value()).push_back(*new_session);
          });

      logs::log(logs::debug, "[API] Re-keyed session {} for resume", session_id);
      auto res = GenericSuccessResponse{.success = true};
      send_http(socket, 200, rfl::json::write(res));
    } else {
      logs::log(logs::warning, "[API] Invalid session_id: {}", resume_req.value().session_id);
      auto res = GenericErrorResponse{.error = "Invalid session_id"};
      send_http(socket, 500, rfl::json::write(res));
    }
  } else {
