...
[WARNING]
====
These scripts are in *beta* — they may not cover every environment. Use at your own risk and please report issues.
====

The quickstart script auto-detects your environment (Proxmox, LXC, Incus, Unraid, TrueNAS, NixOS, Docker, or Podman) and sets up Wolf + Wolf Den automatically:
...
[NOTE]
====
The script requires `sudo` because it performs privileged operations:

* Writing udev rules to `/etc/udev/rules.d/` and reloading them via `udevadm`
* Creating directories under `/etc/wolf/` and `/opt/wolf/`
* Managing containers and services (`docker compose`, `systemctl`, `pct create`, etc.)
* Configuring GPU device passthrough (cgroup rules, LXC device mounts)

**Exception:** On NixOS, the script only generates a `wolf.nix` file and does *not* need root.
====

The script detects your GPU, configures passthrough, installs udev rules, and deploys Wolf with https://github.com/games-on-whales/wolf-den[Wolf Den] (web management UI). When it finishes, follow the on-screen instructions to pair with Moonlight.

See the link:https://github.com/games-on-whales/wolf/tree/stable/quickstart[quickstart README] for the full list of options, supported environments, and troubleshooting.
