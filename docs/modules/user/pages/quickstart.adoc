...
NOTE: `sudo` is required because the script installs udev rules (into `/etc/udev/rules.d/`), creates device passthrough configuration, and manages containers — all of which need root privileges.
