...
onboot: 1
ostype: debian
rootfs: local-lvm:vm-118-disk-0,size=4G
swap: 512
...
From now on, we will work inside the LXC.
...
By now we have all the setup in place, let's create our docker-compose.yml:
...
services:
  wolf:
    ...
    environment:
      ...
      - NVIDIA_DRIVER_VOLUME_NAME=nvidia-driver-vol
      - HOST_APPS_STATE_FOLDER=/etc/wolf
      # This one below is only for multi GPU setup
    ...
    volumes:
      ...
      - /var/run/docker.sock:/var/run/docker.sock:rw
      - /dev/:/dev/:rw
      # This one below uses the /mnt/udev path we created in the LXC config file
