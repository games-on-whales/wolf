...
Wolf can assign independent Moonlight sessions across GPUs using weighted least-loaded scheduling. Leave the `gpus` list empty (the default) to discover `/dev/dri/renderD*` nodes and assign them equally:
...
To omit render nodes from scheduling, set `excluded_gpus` at the top level of the TOML configuration:

----
excluded_gpus = ["/dev/dri/renderD128"]
----

Or configure explicit render nodes and relative capacities. A GPU with weight `10` receives work preferentially over one with weight `7` while it has fewer active sessions relative to its weight:
...
When `gpus` is nonempty, only its listed render nodes are eligible; unlisted GPUs are automatically excluded. `excluded_gpus` also takes precedence over entries in `gpus`. Scheduling compares the projected load after assigning a session, so the highest-weight GPU is selected first when all are idle.

Each session's selected render node is used for the virtual display and runner. `WOLF_RENDER_NODE` remains the fallback for deployments without discoverable/configured GPUs.
